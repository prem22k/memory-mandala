...
const API_URL = import.meta.env.DEV ? 'http://localhost:3000/api/generate' : '/api/generate';

const generatePrompt = async (prompt: string, isJson = false) => {
  try {
    const response = await axios.post(API_URL, { prompt, isJson });
    ...
    throw new Error(`AI API error: ${error.response?.data?.error || error.message}`);
  }
};
